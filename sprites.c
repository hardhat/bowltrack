...
void sprites2C(const char *filename)
{
	...
	fprintf(file,"0x%02x,0x%02x,0x%02x,0x%02x",color[i].y,color[i].x,color[i].patt,color[i].color);
		if((i%4)==3 || i==nextColor-1) fprintf(file,",\n"); else fprintf(file,",");
	}

	fprintf(file,"};\n/* Made %d sprites */\n#define SPATT_LENGTH %d\n",count,count*32);
	fprintf(file,"#define SCOLOR_LENGTH %d\n",count*4);
